 ...
 */
#ifndef CustomPSE_CUH
#define CustomPSE_CUH
...
class CustomPoisson : public Interactor {
public:
  using Kernel = CustomPoisson_ns::Gaussian;
  ...
  CustomPoisson(shared_ptr<ParticleData> pd, Parameters par)
      : CustomPoisson(std::make_shared<ParticleGroup>(pd, "All"), par) {}

  CustomPoisson(shared_ptr<ParticleGroup> pg, Parameters par);

  ~CustomPoisson() {}

  void sum(Computables comp, cudaStream_t st = 0) override {
    sys->log<System::DEBUG2>("[CustomPoisson] Summing interaction");
    farField(st);
    if (comp.force)
      nearFieldForce(st);
    if (comp.energy)
    ...
    if (comp.virial) {
      sys->log<System::EXCEPTION>(
          "[CustomPoisson] Virial functionality not implemented.");
      throw std::runtime_error("[CustomPoisson] not implemented");
    }
  }
